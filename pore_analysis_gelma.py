...
def expand_pore_columns(rows, n_pore_cols):
    """Split the joined `pore_scores` string into pore_1..pore_N columns."""
    max_seen = max((len(r['pore_scores'].split(';')) if r['pore_scores'] else 0)
                   for r in rows) if rows else 0
    if max_seen > n_pore_cols:
        print(f'[WARNING] Some images have {max_seen} target pores, more than '
              f'--n_pore_cols={n_pore_cols}. The extra pore scores are dropped '
              f'from the pore_N columns (the joined "pore_scores" column still '
              f'has all of them). Raise --n_pore_cols to keep them.')
    out = []
    for r in rows:
        scores = r['pore_scores'].split(';') if r['pore_scores'] else []
        rr = dict(r)
        for i in range(n_pore_cols):
            rr[f'pore_{i+1}'] = scores[i] if i < len(scores) else ''
        out.append(rr)
    return out
 
 
def run(data_dir, output_dir, w, min_px, max_px, max_ar, match_overlap_frac,
        close_kernel=21, n_pore_cols=4, csv_name='pore_scores.csv'):
    data_dir   = Path(data_dir)
    output_dir = Path(output_dir)
    ...
    if rows:
        ...
        out_rows = expand_pore_columns(rows, n_pore_cols)
        ...
        print(f'  Feed this straight into build_sample_sf_table.py '
              f'--pore_scores_csv {csv_path.name}')
...
def run_cv_folds(parent_dir, w, min_px, max_px, max_ar, match_overlap_frac,
    ...
    if not fold_dirs:
        n_tif = len(list(parent_dir.glob('*.tif')))
        print(f'[ERROR] No fold directories matching "{fold_glob}" with a '
              f'predictions/ subfolder found in {parent_dir}')
        print()
        print('--cv_parent_dir is only for the cross-validation layout '
              '(fold_0/predictions, fold_1/predictions, ...).')
        if n_tif:
            print(f'This folder holds {n_tif} .tif file(s) directly, so it is a '
                  f'FLAT folder. Use single-folder mode instead:')
            print()
            print(f'    python pore_analysis.py \\')
            print(f'        --data_dir   "{parent_dir}" \\')
            print(f'        --output_dir "{parent_dir}_pore_analysis"')
        else:
            print(f'No .tif files here either. Check the path.')
        return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_pore_cols', type=int, default=4,
        help='Number of pore_N columns written to the scores CSV in BOTH '
             'single-folder and multi-fold mode (default: 4, matching the '
             '3x3 crosshatch geometry -> 2x2 pores)')
    parser.add_argument('--csv_name', default='pore_scores.csv',
        help='Filename for the per-image scores CSV in single-folder mode '
             '(default: pore_scores.csv)')
