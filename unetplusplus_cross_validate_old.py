...
def main(args):
    ...
    # ── cv-level aggregation ──────────────────────────────────────────────────
    def fold_vals(key):
        return [m[key] for m in fold_summaries
                if isinstance(m.get(key), float)]

    iou_pa_folds = fold_vals('mean_iou_pred_annot')
    iou_pt_folds = fold_vals('mean_iou_pred_target')
    ...
    if iou_pa_folds:
        print(f'  IoU (pred|annot)  across folds: '
              f'{np.mean(iou_pa_folds):.3f} ± {np.std(iou_pa_folds):.3f}')
    if iou_pt_folds:
        print(f'  IoU (pred|target) across folds: '
              f'{np.mean(iou_pt_folds):.3f} ± {np.std(iou_pt_folds):.3f}')
