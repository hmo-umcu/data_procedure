...
def process_folder(img_dir, mask_dir, output_dir,
    ...
    mask_files = sorted(
        p for p in mask_dir.glob('*-pred-mask.png')
        if 'visible' not in p.name and 'target' not in p.name
    )
    if not mask_files:
        print('No *-pred-mask.png files found.')
        return
    ...
    for mask_path in mask_files:
        stem     = mask_path.stem.replace('-pred-mask', '')
        ...
        # No annotation JSON in the prediction-only pipeline; well centre
        # comes from HoughCircles on the partially visible well arc.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--drift', action='store_true',
        help='Enable per-well drift correction (ROW_STEP/COL_STEP). Off by '
             'default: the offsets were tuned for the R-GEN 100 run and are '
             'not valid for the R-GEN 200 / GelMA setup until re-tuned.')
