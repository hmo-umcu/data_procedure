...
Prediction-only pipeline: no annotation or labelme JSON needed.

Input test folder must contain:
    {sid}_{row}.tif              original image         REQUIRED
Any {sid}_{row}-target-*.png files present are carried along.
...
def run_test(model_path, data_dir, output_dir, img_size, threshold, use_gpu):
    ...
    # Prediction-only pipeline: no annotation or labelme JSON. Copy the raw
    # TIF so downstream (draw_target_geometry.py, pore_analysis.py) can read
    # it from the output folder. Target files, if present from an earlier
    # run, are carried along.
    COPY_SUFFIXES = ['-target-overlay.png', '-target-mask.png']
    ...
    fieldnames = [
        ...
        'pred_mask', 'pred_visible', 'pred_px',
    ]
