...
Parameters:
  Pressure_kPa    : 60 – 130 kPa  (step 5)
  NozzleSpeed_mms : 5  – 15  mm/s (step 1)
  Zoffset_mm      : 0.4 – 0.9 mm  (step 0.1)

Note: Temperature is excluded from the optimization space and set
manually in the R-Gen Architect UI before each print run.
...
PARAMS = {
    ...
    "Zoffset_mm":      {"min": 0.2, "max": 0.7, "step": 0.1, "decimals": 2},
}
...
# ── 5. Sort by Pressure, then NozzleSpeed, then Zoffset ──────────────────────
df = df.sort_values(
    by=["Pressure_kPa", "NozzleSpeed_mms", "Zoffset_mm"]
